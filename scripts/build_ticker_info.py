...
# Keep normal ETFs, leveraged ETFs, inverse ETFs, bull/bear ETFs, daily ETFs.
# Remove only these ETF strategy/product types.
BAD_ETF_KEYWORDS = [
    "microsectors",
    " etn",
    " etns",
    ...
    with open(EXCLUDED_SYMBOLS_PATH, "w", encoding="utf-8") as f:
        for row in excluded_rows:
            f.write(
                f"{row.get('Symbol', '')}|"
                f"{row.get('ETF', '')}|"
                f"{row.get('_exclude_reason', '')}|"
                f"{row.get('Security Name', '')}\n"
            )
        ...
        "sample": [
            {
                ...
                "reason": row.get("_exclude_reason", ""),
                "security_name": row.get("Security Name", "")
            }
            for row in excluded_rows[:200]
        ]
    }
...
def log_etf_filter_summary(before_count: int, after_count: int, excluded_rows: List[Dict[str, str]]):
    ...
    logging.info("Excluded report: %s", EXCLUDED_SYMBOLS_PATH)
    logging.info("Excluded summary: %s", EXCLUDED_SUMMARY_PATH)
    logging.info("------------------------------------------------------------")
    logging.info("ETF FILTER BREAKDOWN")
    for reason, count in sorted(reason_counter.items()):
        logging.info("%-25s : %s", reason, count)
    logging.info("------------------------------------------------------------")
    logging.info("ALL EXCLUDED ETF PRODUCTS")
    for row in excluded_rows:
        logging.info(
            "%-8s | %-18s | %s",
            row.get("Symbol", ""),
            row.get("_exclude_reason", ""),
    ...
    print("============================================================")
    print(f"NASDAQ eligible symbols before ETF filter: {before_count}")
    print(f"Excluded ETF products: {len(excluded_rows)}")
    print(f"Remaining symbols after ETF filter: {after_count}")
    print(f"Excluded report: {EXCLUDED_SYMBOLS_PATH}")
    print(f"Excluded summary: {EXCLUDED_SUMMARY_PATH}")
    print("------------------------------------------------------------")
    print("ETF FILTER BREAKDOWN")
    for reason, count in sorted(reason_counter.items()):
        print(f"{reason:<25} : {count}")
    print("------------------------------------------------------------")
    print("ALL EXCLUDED ETF PRODUCTS")
    for row in excluded_rows:
        print(
            f"{row.get('Symbol', ''):<8} | "
            f"{row.get('_exclude_reason', ''):<18} | "
            f"{row.get('Security Name', '')}"
        )
    print("============================================================")
...
def process_batch(batch, existing, nasdaq_data_map):
    ...
    for symbol in tqdm(batch, desc="Symbols", leave=False):
        ...
        if sector and industry and quality(sector, industry):
            ...
            if (
                not prev
                or prev["info"]["sector"] != sector
                or prev["info"]["industry"] != industry
            ):
                existing[symbol] = {
                    "info": {
                        "industry": industry,
...
def main(
    ...
    todo = [
        s for s in symbols_slice
        if needs_update(s, existing, force_refresh)
    ]
    ...
    for idx, batch in enumerate(tqdm(batches, desc="Processing Batches"), 1):
        ...
        logging.info(
            "Batch %d/%d - Updated: %d | Unresolved: %d | Updated total: %d",
            idx,
            len(batches),
            updated,
            len(unresolved),
    ...
    if RETRY_SUBPASS and all_unresolved:
        ...
        if unresolved_unique:
            ...
            for batch in tqdm(
                list(partition(unresolved_unique, BATCH_SIZE)),
                desc="Retry Batches"
            ):
                ...
                logging.info(
                    "Retry batch updated: %d | still unresolved: %d | Updated total: %d",
                    updated,
                    len(retry_unres),
                    updated_total
                )
    ...
    unresolved_final = sorted(
        sym for sym, v in existing.items()
        if is_incomplete(v)
    )
    ...
    logging.info(
        "Done. Total entries: %d | Unresolved: %d | Updated this run: %d | Elapsed: %.1fs",
        len(existing),
        len(unresolved_final),
        updated_total,
        elapsed
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Build / update ticker_info.json from NASDAQ master list."
    )
