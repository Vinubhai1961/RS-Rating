...
EXCLUDED_SYMBOLS_PATH = "data/excluded_symbols.txt"
EXCLUDED_SUMMARY_PATH = "data/excluded_symbols_summary.json"
...
# Keep normal ETFs... Remove only these
BAD_ETF_KEYWORDS = [
    "microsectors", " etn", " etns", "exchange traded note", "exchange traded notes",
    "autocallable", "buffer", "weeklypay", "yield premium", "covered call", "income strategy",
]

BAD_NON_STOCK_KEYWORDS = [
    "senior notes", "subordinated notes", "notes due", "debentures", "preferred stock",
    "preference shares", "depositary shares", "trust preferred", "warrant", "rights", "units",
]
...
def matched_bad_etf_keyword(row: Dict[str, str]):
    etf_flag = str(row.get("ETF", "N")).strip().upper()
    if etf_flag != "Y":
        return None
    name = str(row.get("Security Name", "")).lower()
    for keyword in BAD_ETF_KEYWORDS:
        if keyword in name:
            return keyword.strip()
    return None


def write_excluded_reports(excluded_rows: List[Dict[str, str]]):
    reason_counter = Counter(row.get("_exclude_reason", "unknown") for row in excluded_rows)
    with open(EXCLUDED_SYMBOLS_PATH, "w", encoding="utf-8") as f:
        for row in excluded_rows:
            f.write(f"{row.get('Symbol', '')}|{row.get('ETF', '')}|{row.get('_exclude_reason', '')}|{row.get('Security Name', '')}\n")
    summary = {
        "total_excluded": len(excluded_rows),
        "by_reason": dict(reason_counter),
        "sample": [{"symbol": row.get("Symbol", ""), "reason": row.get("_exclude_reason", ""), "security_name": row.get("Security Name", "")} for row in excluded_rows[:200]]
    }
    with open(EXCLUDED_SUMMARY_PATH, "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, sort_keys=True)


def log_etf_filter_summary(before_count: int, after_count: int, excluded_rows: List[Dict[str, str]]):
    reason_counter = Counter(row.get("_exclude_reason", "unknown") for row in excluded_rows)
    logging.info("============================================================")
    logging.info("ETF FILTER SUMMARY")
    logging.info("============================================================")
    logging.info("NASDAQ eligible symbols before ETF filter: %s", before_count)
    logging.info("Excluded ETF products: %s", len(excluded_rows))
    logging.info("Remaining symbols after ETF filter: %s", after_count)
    logging.info("------------------------------------------------------------")
    logging.info("ETF FILTER BREAKDOWN")
    for reason, count in sorted(reason_counter.items()):
        logging.info("%-25s : %s", reason, count)
    logging.info("============================================================")


def apply_etf_filter(symbols_data: List[Dict[str, str]]) -> List[Dict[str, str]]:
    before_count = len(symbols_data)
    filtered = []
    excluded = []
    for row in symbols_data:
        reason = matched_bad_etf_keyword(row)
        if reason:
            row = dict(row)
            ...
            filtered.append(row)
    write_excluded_reports(excluded)
    log_etf_filter_summary(before_count, len(filtered), excluded)
    return filtered
...
def fetch_nasdaq_symbols() -> List[Dict[str, str]]:
    ...
    logging.info("Retrieved %d eligible symbols before ETF filtering.", len(symbols_data))
    symbols_data = apply_etf_filter(symbols_data)
    logging.info("Final eligible symbols after ETF filtering: %d", len(symbols_data))
    return symbols_data
...
def process_batch(batch, existing, nasdaq_data_map):
    ...
    for symbol in tqdm(batch, desc="Symbols", leave=False):
        ...
        if symbol in failed_original or yahoo_symbol(symbol) in failed:
            if symbol not in existing:
                existing[symbol] = {"info": {"industry": default_industry, "sector": default_sector, "type": default_type}}
        ...
        if sector and industry and quality(sector, industry):
            ...
            if symbol not in existing:
                ...
                existing[symbol] = {"info": {"industry": fallback_industry, "sector": fallback_sector, "type": fallback_type}}
            unresolved.append(symbol)
...
def print_partition_summary(summary: Dict):
    logging.info("=" * 70)
    logging.info("BUILD TICKER INFO - PARTITION SUMMARY")
    logging.info("=" * 70)
    for key, value in summary.items():
        logging.info(f"{key.replace('_', ' ').title():<30}: {value}")
    logging.info("=" * 70)
...
def main(
    ...
    logging.info("🚀 Starting ticker info build - Partition %s/%s", part_index if part_index is not None else "FULL", part_total)
    ...
    if part_index is not None and part_total is not None:
        ...
        logging.info("Partition %d/%d: %d symbols", part_index + 1, part_total, len(symbols_slice))
    else:
    ...
    for idx, batch in enumerate(tqdm(batches, desc="Processing Batches"), 1):
        ...
        logging.info("Batch %d/%d - Updated: %d | Unresolved: %d | Updated total: %d", idx, len(batches), updated, len(unresolved), updated_total)
        if idx < len(batches):
    ...
    if RETRY_SUBPASS and all_unresolved:
        unresolved_unique = sorted(set(sym for sym in all_unresolved if is_incomplete(existing.get(sym, {}))))
        if unresolved_unique:
            logging.info("Retry sub-pass for %d unresolved symbols ...", len(unresolved_unique))
            for batch in tqdm(list(partition(unresolved_unique, BATCH_SIZE)), desc="Retry Batches"):
                updated, retry_unres = process_batch(batch, existing, nasdaq_data_map)
                updated_total += updated
                save(existing, part_index)
                logging.info("Retry batch updated: %d | still unresolved: %d", updated, len(retry_unres))
    ...
    print_partition_summary(summary)

    logging.info("✅ Partition completed successfully.")
