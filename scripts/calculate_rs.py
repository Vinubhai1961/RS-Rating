...
def create_empty_csvs(output_dir, log_file):
    """Create empty CSVs if data processing fails."""
    logging.info("Creating empty CSVs due to data processing failure")
    os.makedirs(output_dir, exist_ok=True)
    pd.DataFrame(columns=["Rank", "Ticker", "Price", "Sector", "Industry", "Relative Strength",
                          "Percentile", "1 Month Ago", "3 Months Ago", "6 Months Ago"]).to_csv(
...
def main(input_file, min_percentile, reference_ticker, output_dir, log_file):
    ...
    if not os.path.exists(input_file):
        ...
        create_empty_csvs(output_dir, log_file)
        return
    ...
    try:
        ...
        logging.error(f"Failed to read reference ticker {reference_ticker} from ArcticDB: {str(e)}")
        create_empty_csvs(output_dir, log_file)
        return
    ...
    for ticker in tqdm(tickers, desc=f"Calculating RS (~{est_time_per_ticker*1000:.2f}ms/ticker)"):
        ...
        try:
            ...
            logging.info(f"{ticker}: Failed to process RS calculation ({str(e)})")
    ...
    try:
        ...
        logging.error(f"Failed to compute percentiles: {str(e)}")
        create_empty_csvs(output_dir, log_file)
        return
    ...
    try:
        ...
        logging.error(f"Failed to finalize rs_stocks.csv: {str(e)}")
        create_empty_csvs(output_dir, log_file)
        return
    ...
    try:
        ...
        logging.error(f"Failed to adjust ETF metadata: {str(e)}")
        create_empty_csvs(output_dir, log_file)
        return
    ...
    try:
        ...
        logging.error(f"Failed to create rs_industries.csv: {str(e)}")
        create_empty_csvs(output_dir, log_file)
        return
    ...
    try:
        ...
        logging.error(f"Failed to create RSRATING.csv: {str(e)}")
        create_empty_csvs(output_dir, log_file)
        return
