...
def get_atr_adr(ticker: str, lib, period: int = 14):
    """Backward-compatible ATR/ADR helper that reads from ArcticDB."""
    try:
        data = lib.read(ticker).data
        return calculate_atr_adr_from_dataframe(data, ticker, period)
...
def calculate_atr_adr_from_dataframe(data: pd.DataFrame, ticker: str, period: int = 14):
    ...
    ArcticDB dataframe. This avoids a second lib.read(ticker) call per symbol.
    """
    try:
...
def load_metadata(metadata_file):
    """
    Robust USA metadata loader. Supports both:
      1. list format: [{"ticker": "AAPL", "info": {...}}, ...]
      2. dict format: {"AAPL": {"info": {...}}, ...}
    Preserves USA fields including Type and Earning_Date.
    """
...
def main(arctic_db_path, reference_ticker, output_dir, log_file, metadata_file=None, percentiles=None, debug=False):
    ...
    # Runtime-only data quality counters. Printed/logged only; no extra committed file.
    quality_stats = {
        "total_processed": 0,
        "valid_rs": 0,
        "missing_rs": 0,
    ...
    for ticker in tqdm(tickers, desc="Calculating RS"):
        ...
        try:
            ...
            if len(closes) < 2:
                ...
                rs_results.append((ticker, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, None, np.nan, np.nan))
                continue

            sma50, sma200, sma10w, sma30w = calculate_smas(closes)

            rs = relative_strength(closes, ref_closes)
            # === FAST ATR & ADR: use already-loaded dataframe ===
    ...
    final_columns = ["Rank", "Ticker", "Price", "DVol", "Sector", "Industry", "RS Percentile", "1M_RS Percentile", "3M_RS Percentile", "6M_RS Percentile", "ATR", "ADR", "AvgVol", "AvgVol10", "52WKH", "52WKL", "MCAP", "IPO", "SMA50", "SMA200", "SMA10W", "SMA30W"]

    # Safe Earning_Date handling after merge
    if "Earning_Date" in df_stocks.columns:
        # Insert Earning_Date after IPO
        try:
            idx = final_columns.index("IPO") + 1
            final_columns.insert(idx, "Earning_Date")
        except ValueError:
    ...
    print("\n=== USA RS DATA QUALITY SUMMARY ===")
    for key, value in quality_stats.items():
        print(f"{key}: {value}")
        logging.info(f"QUALITY {key}: {value}")
