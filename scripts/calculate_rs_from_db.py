...
def main(arctic_db_path, reference_ticker, output_dir, log_file, metadata_file=None, percentiles=None, debug=False, hve_output_dir="HVE", vol9m_output_dir="9M_Vol", min_daily_volume=9_000_000):
    ...
    for ticker in tqdm(tickers, desc="Calculating RS"):
        ...
        try:
            ...
            quality_stats["missing_rs"] += 1
            # 22 values, must match the df_stocks column list exactly
            rs_results.append((
                ticker,            # Ticker
                np.nan,            # RS
                np.nan,            # 1M_RS
    ...
    # so rs_stocks.csv always has the full header.
    ...
    # Last-resort fallback: derive Prev_Close from Last_Close / Gap when blank
    if "Prev_Close" in df_stocks.columns:
        df_stocks["Prev_Close"] = pd.to_numeric(df_stocks["Prev_Close"], errors="coerce").round(4)

    missing_prev = int(df_stocks["Prev_Close"].isna().sum())
    missing_sma20 = int(pd.to_numeric(df_stocks["SMA20"], errors="coerce").isna().sum())
    logging.info(f"Prev_Close missing: {missing_prev:,} | SMA20 missing: {missing_sma20:,}")
    ...
    for required in ["Prev_Close", "SMA20"]:
        if required not in available_cols:
            raise RuntimeError(f"Required column {required} missing from rs_stocks output")
    os.makedirs(output_dir, exist_ok=True)
