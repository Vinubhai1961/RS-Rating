...
def main(arctic_db_path, reference_ticker, output_dir, log_file, metadata_file=None, percentiles=None):
    ...
    for col in ["RS", "1M_RS", "3M_RS", "6M_RS"]:
        ...
        df_stocks[f"{col} Percentile"] = df_stocks[f"{col} Percentile"].fillna(0)
    ...
    # Debug: Log NaN counts before processing
    logging.info("Pre-agg check for NaNs in df_industries percentiles:")
    for col in ["RS Percentile", "1M_RS Percentile", "3M_RS Percentile", "6M_RS Percentile"]:
        nan_count = df_industries[col].isna().sum()
        inf_count = np.isinf(df_industries[col]).sum()
        logging.info(f"  {col}: {nan_count} NaNs, {inf_count} infs")
        if nan_count > 0:
            logging.warning(f"  Industries with NaN in {col}: {df_industries[df_industries[col].isna()]['Industry'].tolist()}")
