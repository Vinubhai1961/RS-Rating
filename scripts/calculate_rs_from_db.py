    ...
    for ticker in tqdm(tickers, desc="Calculating RS"):
        ...
        try:
            ...
            if len(closes) < 2:
                ...
                rs_results.append((ticker, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, len(closes), np.nan, gap_pct, latest_volume, vol9m_plus, hve_record.get("HVE", "NO"), hve_record.get("HVE Date", ""), hve_record.get("HVE Volume", np.nan), None, np.nan, np.nan))
                continue
            ...
            rs_results.append((ticker, rs, rs_1m, rs_3m, rs_6m, sma50, sma200, sma10w, sma30w, len(closes), round(float(closes.iloc[-1]), 4), gap_pct, latest_volume, vol9m_plus, hve_record.get("HVE", "NO"), hve_record.get("HVE Date", ""), hve_record.get("HVE Volume", np.nan), earning_date, atr, adr))
            ...
            quality_stats["missing_rs"] += 1
            rs_results.append((ticker, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, "NO", "", np.nan, None, np.nan, np.nan))

    df_stocks = pd.DataFrame(rs_results, columns=["Ticker", "RS", "1M_RS", "3M_RS", "6M_RS", "SMA50", "SMA200", "SMA10W", "SMA30W", "History_Days", "Last_Close", "Gap (%)", "Latest Volume", "9M+ Volume", "HVE", "HVE Date", "HVE Volume", "Earning_Date", "ATR", "ADR"])
